...
So now that you have a webhook added to the site. You now need to parse the webhook. In order to successfully parse a webhook sent by Infinity Bot List, here is the algorithm you must implement. *Note that all examples are provided in JS as of right now*.
